...
public class SkipList<T> {
    ...
    public void printRow() {
        SkipNode last = headNode;
        while (last.down != null) {
            last = last.down;
        }
        last = last.right;
        while (last != null) {
            System.out.printf("%-5s", last.key + "->");
            last = last.right;
        }
        System.out.println();
    }
    ...
    public static void main(String[] args) {
        int deleteKey1 = 4, deleteKey2 = 8;
        SkipList<Integer> list = new SkipList<Integer>();
        for (int i = 1; i < 20; i++) {
            list.add(new SkipNode(i, 666));
        }
        list.add(new SkipNode(5, 777));
        list.add(new SkipNode(5, 999));
        list.printList();
        list.delete(deleteKey1);
        list.delete(deleteKey2);
        System.out.println("After Delete ... " + deleteKey1 + ", " + deleteKey2);
        list.printList();

        System.out.println();
        list.printRow();

        SkipNode node = list.search(5);
